                    ...
                    DragGesture()
                        .onChanged { gestureValue in
                            ...
                            if self.options.contains(.forceAdjacentValue) {
                                let computedUpperBound = max(computedLowerBound, CGFloat(configuration.range.wrappedValue.upperBound))
                                configuration.range.wrappedValue = computedLowerBound...computedUpperBound
                            } else {
                                let computedLowerBound = min(computedLowerBound, CGFloat(configuration.range.wrappedValue.upperBound))
                ...
                .gesture(
                    DragGesture()
                        .onChanged { gestureValue in
                            ...
                            if self.options.contains(.forceAdjacentValue) {
                                let computedLowerBound = min(computedUpperBound, CGFloat(configuration.range.wrappedValue.lowerBound))
                                configuration.range.wrappedValue = computedLowerBound...computedUpperBound
                            } else {
                                let computedUpperBound = max(computedUpperBound, CGFloat(configuration.range.wrappedValue.lowerBound))
